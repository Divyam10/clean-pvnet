...
class BPnP(torch.autograd.Function):
    ...
    def forward(ctx, pts2d, pts3d, K, ini_pose=None):
        ...
        for i in range(bs):
            pts2d_i_np = np.ascontiguousarray(
                pts2d[i].detach().cpu()).reshape((n, 1, 2))
            if ini_pose is None:
                _, rvec0, T0, _ = cv.solvePnPRansac(objectPoints=pts3d_np, imagePoints=pts2d_i_np, cameraMatrix=K_np,
                                                    distCoeffs=None, flags=cv.SOLVEPNP_ITERATIVE, confidence=0.9999, reprojectionError=3)
            else:
                rvec0 = np.array(ini_pose[i, 0:3].cpu().view(3, 1))
                T0 = np.array(ini_pose[i, 3:6].cpu().view(3, 1))
            _, rvec, T = cv.solvePnP(objectPoints=pts3d_np, imagePoints=pts2d_i_np, cameraMatrix=K_np,
                                     distCoeffs=None, flags=cv.SOLVEPNP_ITERATIVE, useExtrinsicGuess=True, rvec=rvec0, tvec=T0)
            angle_axis = torch.tensor(
                rvec, device=device, dtype=torch.float).view(1, 3)
        ...
        for i in range(bs):
            ...
            for j in range(m):
                ...
                R = kn.angle_axis_to_rotation_matrix(
                    P_6d_flat[0:m-3].view(1, 3))

                P = torch.cat((R[0, 0:3, 0:3].view(3, 3),
                              P_6d_flat[m-3:m].view(3, 1)), dim=-1)
                KP = torch.mm(K_flat.view(3, 3), P)
                pts2d_i = pts2d_flat.view(n, 2).transpose(0, 1)
                pts3d_i = torch.cat(
                    (pts3d_flat.view(n, 3), torch.ones(n, 1, device=device)), dim=-1).t()
                proj_i = KP.mm(pts3d_i)
                Si = proj_i[2, :].view(1, n)

                r = pts2d_i*Si-proj_i[0:2, :]
                coefs = get_coefs(P_6d_flat.view(
                    1, 6), pts3d_flat.view(n, 3), K_flat.view(3, 3))
...
class BPnP_m3d(torch.autograd.Function):
    ...
    def forward(ctx, pts2d, pts3d, K, ini_pose=None):
        ...
        for i in range(bs):
            pts2d_i_np = np.ascontiguousarray(
                pts2d[i].detach().cpu()).reshape((n, 1, 2))
            pts3d_i_np = np.ascontiguousarray(
                pts3d[i].detach().cpu()).reshape((n, 3))
            if ini_pose is None:
                _, rvec0, T0, _ = cv.solvePnPRansac(objectPoints=pts3d_i_np, imagePoints=pts2d_i_np, cameraMatrix=K_np,
                                                    distCoeffs=None, flags=cv.SOLVEPNP_ITERATIVE, confidence=0.9999, reprojectionError=1)
            else:
                rvec0 = np.array(ini_pose[i, 0:3].cpu().view(3, 1))
                T0 = np.array(ini_pose[i, 3:6].cpu().view(3, 1))
            _, rvec, T = cv.solvePnP(objectPoints=pts3d_i_np, imagePoints=pts2d_i_np, cameraMatrix=K_np,
                                     distCoeffs=None, flags=cv.SOLVEPNP_ITERATIVE, useExtrinsicGuess=True, rvec=rvec0, tvec=T0)
            angle_axis = torch.tensor(
                rvec, device=device, dtype=torch.float).view(1, 3)
            T = torch.tensor(T, device=device, dtype=torch.float).view(1, 3)
...
class BPnP_fast(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        for i in range(bs):
            ...
            coefs = get_coefs(P_6d[i].view(1, 6), pts3d,
                              K, create_graph=False).detach()
...
def get_coefs(P_6d, pts3d, K, create_graph=True):
    ...
    for k in range(2):
        ...
        y_grad = torch.autograd.grad(
            proj[:, :, k], y, vec, retain_graph=True, create_graph=create_graph)
...
def batch_project(P, pts3d, K, angle_axis=True):
    ...
    if angle_axis:
        R_out = kn.geometry.angle_axis_to_rotation_matrix(
            P[:, 0:3].view(bs, 3))
